...
def scrapSeason(imdbID, season_no):
    seasons_url = imdb_url + "title/"+imdbID + \
        "/episodes/?season="+str(season_no)
    r = requests.get(url=seasons_url)
    soup = BeautifulSoup(r.text, 'html.parser')
    episodes = []
    episodetags = soup.find('div', class_=['eplist']).findAll(
        'div', class_=['list_item'])
    for e in episodetags:
        ...
        try:
            ...
            episode['air_date'] = e.find(
                'div', {'class': 'airdate'}).string.strip()
            episode['title'] = e.find('strong').find('a')['title']
            episode['episode_url'] = e.find('strong').find('a')['href']
            episode['rating_value'] = e.find(
                'span', {'class': 'ipl-rating-star__rating'}).string.strip()
            episode['rating_value'] = e.find(
                'span', {'class': 'ipl-rating-star__total-votes'}).string.strip()
            episode['episode_plot'] = e.find(
                'div', {'class': 'item_description'}).string.strip()
...
def scrapeTv(imdbID):
    ...
    try:
        name = soup.find(
            'div', {'class': 'subpage_title_block__right-column'}).find('a').string.strip()
    except:
        pass
    ...
    try:
        air_years = soup.find(
            'div', {'class': 'subpage_title_block__right-column'}).find('span').string.strip()
    except:
        pass
